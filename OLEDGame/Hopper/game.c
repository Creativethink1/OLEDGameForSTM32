...
/*  Local Variables  */
	...
	static const uint8_t imgPlayers[][8] = { // 8x8 x16
	...
	static const uint8_t imgFigures[][16] = { // 8x12 x10
	...
	static const uint8_t imgStart[] = { // 50x12 "Ready?"
	...
	static const uint8_t imgClear[] = { // 68x10 "Excellent!"
	...
	static const uint8_t imgOver[] = { // 76x10 "Game Over"
    ...
    if (state == STATE_GAME || state == STATE_OVER) {
        ...
        if (state == STATE_GAME) {
            ...
            if (floorAry[floorIdxFirst].z < 0) {
              ...
              //  dprintln(score);
            } else if (ignoreCnt == 0 && ButtonDown(A_BUTTON | B_BUTTON)) {
                state = STATE_PAUSE;
                //dprintln("Pause");
            }
        } else {
            counter--;
            if (ignoreCnt == 0 && ButtonDown(A_BUTTON | B_BUTTON)) {
                initLevel(true);
            }
        }
    }
...
{
    int vx = 0, vy = 0, vr;
    if (ButtonPressed(LEFT_BUTTON)) vx--;
    if (ButtonPressed(RIGHT_BUTTON)) vx++;
    if (ButtonPressed(UP_BUTTON)) vy--;
    if (ButtonPressed(DOWN_BUTTON)) vy++;
    vr = (vx != 0 && vy != 0) ? 384 : 512;
    playerX += vx * vr;
    playerY += vy * vr;

    if (vx != 0 || vy != 0) {
        uint8_t targetRotate = (uint8_t) (atan2((float)vy, (float)vx) * 128.0 / PI);
...
static void drawPlayer(void)
{
    ...
    if (isBlink) {
        for (int p = 0; p < 9; p++) {
			OLED_DrawBitmap(x + (p % 3) - 1, y + (p / 3) - 1, imgPlayers[rotate], 8, 8, BLACK);
        }
    } else {
        uint8_t p = pgm_read_byte(posEyesMask + rotate);
        FillRect2(x + (p % 16), y + (p / 16), 4, 4, BLACK);
    }
       OLED_DrawBitmap(x, y, imgPlayers[rotate], 8, 8, WHITE);
}
...
static void drawFloors(void)
{
    for (int i = 0; i < FLOORS_MANAGE; i++) {
        ...
        if (type == FLRTYPE_GOAL) {
            FillRect2(x, y, s, s, (isBlink) ? WHITE : BLACK);
        } else {
            FillPatternedRect(x, y, s, s, imgFloor[type - 1]);
        }
        DrawRect(x, y, s, s, WHITE);
    }
}
...
static void drawStrings(void)
{
    if (state == STATE_START) {
        OLED_DrawBitmap(39, 12, imgStart, 50, 12, WHITE);
    } else {
        OLED_PrintCharEx(0, 0, F("SCORE"));
        drawFigure(0, 6, score, ALIGN_LEFT);
        OLED_PrintCharEx(98, 0, F("LEVEL"));
        drawFigure(120, 6, level + 1, ALIGN_RIGHT);
        if (state == STATE_CLEAR) {
            OLED_DrawBitmap(30, 12, imgClear, 68, 12, WHITE);
            OLED_PrintCharEx(22, 46, F("TRY NEXT LEVEL"));
        } else if (state == STATE_OVER) {
			OLED_DrawBitmap(random2(25, 28), random2(11, 14), imgOver, 76, 12, WHITE);
            if (isHiscore && counter % 8 < 4) {
                OLED_PrintCharEx(31, 46, F("NEW RECORD!"));
            }
        } else if (state == STATE_PAUSE) {
            OLED_PrintCharEx(49, 46, F("PAUSE"));
        }
...
static int  drawFigure(int16_t x, int16_t y, int value, uint8_t align)
{
    ...
    OLED_DrawBitmap(x + k, y, imgFigures[value % 10], 8, 16, WHITE);
    return k + 8 - align * 4;
}
