#include "hopper.h"

/*  Defines  */

enum {
    STATE_MENU = 0,
    STATE_RECORD,
    STATE_CREDIT,
};

enum {
    MENU_START = 0,
    MENU_SOUND,
    MENU_RECORD,
    MENU_CREDIT,
};

enum {
    RECORD_NOT_READ = 0,
    RECORD_INITIAL,
    RECORD_STORED,
};

#define EEPROM_ADDR_BASE    800
#define EEPROM_SIGNATURE    0x024E424FUL // "OBN\x02"

/*  Typedefs  */

typedef struct
{
    const uint8_t *pImg;
    uint8_t width;
} TITLE_LETTER;

/*  Local Functions  */

static void initObjects(void);
static void moveObjects(void);
static bool handleButtons(void);
//static void setSound(bool enabled);
//static void playSound1(void);
//static void playSound2(void);
//static void readRecord(void);

static void drawTitleMenu(void);
static void drawTitleRecord(void);
static void drawTitleCredit(void);

/*  Local Variables  */

	static const uint8_t imgMan[][128] = { // 32x32 x4
    {
        ,
    },{
        ,
    },{
        ,
    },{
        ,
    }
};

	static const uint8_t imgLetterH[] = { // 16x16 "H"
    0x03, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC3, 0xC0, 0xC0, 0xC3, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x03,
    0xC0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC1, 0x01, 0x01, 0xC1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC0,
};

	static const uint8_t imgLetterO[] = { // 13x11 "o"
    0xF0, 0xFC, 0xFE, 0xFE, 0xFF, 0x83, 0x03, 0x0F, 0xFF, 0xFF, 0xFE, 0xFC, 0xF8,
    0x00, 0x01, 0x03, 0x07, 0x07, 0x07, 0x06, 0x06, 0x07, 0x03, 0x03, 0x01, 0x00,
};

	static const uint8_t imgLetterP[] = { // 14x15 "p"
    0x0C, 0xFE, 0xFE, 0xFF, 0xFF, 0xFF, 0x02, 0x03, 0x07, 0xFF, 0xFF, 0xFF, 0xFE, 0xF8,
    0x30, 0x3F, 0x7F, 0x7F, 0x7F, 0x7F, 0x62, 0x06, 0x07, 0x07, 0x07, 0x03, 0x01, 0x00,
};

	static const uint8_t imgLetterE[] = { // 11x11 "e"
    0xF8, 0xFC, 0xFE, 0xFF, 0xFF, 0x33, 0x33, 0x3F, 0x3E, 0x3E, 0x38,
    0x00, 0x01, 0x03, 0x07, 0x07, 0x07, 0x06, 0x06, 0x06, 0x03, 0x03,
};

	static const uint8_t imgLetterR[] = {
    0x06, 0xFE, 0xFE, 0xFF, 0xFF, 0xFF, 0x0C, 0x1F, 0x1F, 0x1F, 0x0E,
    0x06, 0x07, 0x07, 0x07, 0x07, 0x07, 0x06, 0x06, 0x00, 0x00, 0x00,
};

	static const TITLE_LETTER letterInfos[] = {
    { imgLetterH, 16 },{ imgLetterO, 13 },{ imgLetterP, 14 },
    { imgLetterP, 14 },{ imgLetterE, 11 },{ imgLetterR, 11 },
};

	static const char menuText[] = "START GAME\0SOUND \0RECORD\0CREDIT";
	static const char creditText[] = "- " APP_TITLE " -\0\0" APP_RELEASED \
        "\0PROGREMMED BY OBONO\0\0THIS PROGRAM IS\0" "RELEASED UNDER\0" "THE MIT LICENSE.";

static uint8_t  state;
static bool     toDraw;
static bool     sound;
static int8_t   menuPos;
static int8_t   manX, manVy, manType;
static int8_t   letterY[6];
static int8_t   letterVy[6];
static uint16_t lastScore = 0;
static uint8_t  recordState = RECORD_NOT_READ;
static uint16_t hiScore[10];
static uint16_t playCount;
static uint32_t playFrames;

/*---------------------------------------------------------------------------*/
/*                              Main Functions                               */
/*---------------------------------------------------------------------------*/

void initTitle(void)
{
    state = STATE_MENU;
    menuPos = MENU_START;
    toDraw = true;
    initObjects();
//    setSound(arduboy.audio.enabled());
//    if (recordState == RECORD_NOT_READ) {
//        readRecord();
//    }
}

bool updateTitle(void)
{
    bool ret = false;
    if (state == STATE_MENU) {
        moveObjects();
        ret = handleButtons();
        toDraw = true;
    } else {
        if (ButtonDown(A_BUTTON | B_BUTTON)) {
            state = STATE_MENU;
            //playSound2();
            toDraw = true;
        }
    }


    randomSeed(rand() ^ micros()); // Shuffle random
    return ret;
}

void drawTitle(void)
{
    if (toDraw) {
        OLED_ClearDisplay();
        switch (state) {
        case STATE_MENU:
            drawTitleMenu();
            break;
        case STATE_RECORD:
            drawTitleRecord();
            break;
        case STATE_CREDIT:
            drawTitleCredit();
            break;
        }
        toDraw = false;
    }
}

uint8_t setLastScore(int score, uint32_t frames)
{
    lastScore = score;

    /*  Updarte best 10  */
    int r = 10;
    uint16_t h;
    while (r > 0 && (h = hiScore[r - 1]) < score) {
        if (--r < 9) hiScore[r + 1] = h;
    }
    if (r < 10) hiScore[r] = score;
    playCount++;
    playFrames += frames;

    /*  Store record to EEPROM  */
    if (recordState == RECORD_INITIAL) {
        //arduboy.eepSeek(EEPROM_ADDR_BASE);
       // arduboy.eepWrite32(EEPROM_SIGNATURE);
    }
    //arduboy.eepSeek(EEPROM_ADDR_BASE + 4 + r * 2);
    for (int i = r; i < 10; i++) {
       // arduboy.eepWrite16(hiScore[i]);
    }
   // arduboy.eepWrite16(playCount);
   // arduboy.eepWrite32(playFrames);

    uint16_t checkSum = (EEPROM_SIGNATURE & 0xFFFF) + (EEPROM_SIGNATURE >> 16) * 2;
    for (int i = 0; i < 10; i++) {
        checkSum += hiScore[i] * (i + 3);
    }
    checkSum += playCount * 13;
    checkSum += (playFrames & 0xFFFF) * 14 + (playFrames >> 16) * 15;
  //  arduboy.eepWrite16(checkSum);

    recordState = RECORD_STORED;
    return r;
}

/*---------------------------------------------------------------------------*/
/*                             Control Functions                             */
/*---------------------------------------------------------------------------*/

static void initObjects(void)
{
    manVy = 32;
    for (int i = 0; i < 6; i++) {
        letterY[i] = -72;
        letterVy[i] = 0;
    }
}

static void moveObjects(void)
{
    if (manVy < 32) {
        manVy++;
    } else  {
        manX = random(25);
        manVy = -random2(32, 97);
        manType = random(4);
    }

    for (int i = 0; i < 6; i++) {
        int8_t y = letterY[i], vy = letterVy[i];
        y += ++vy;
        if (y >= 64) {
            y = 64;
            vy = -sqrt((float)random2(64, 196));
        }
        letterY[i] = y;
        letterVy[i] = vy;
    }
}

static bool handleButtons(void)
{
    bool ret = false;
    if (ButtonDown(UP_BUTTON)) {
        if (menuPos-- == MENU_START) menuPos = MENU_CREDIT;
       // playSound1();
    }
    if (ButtonDown(DOWN_BUTTON)) {
        if (menuPos++ == MENU_CREDIT) menuPos = MENU_START;//先参与运算后自增
       // playSound1();
    }
    if (ButtonDown(A_BUTTON | B_BUTTON)) {
        switch (menuPos) {
        case MENU_START:
            ret = true;
           // arduboy.audio.saveOnOff();
            break;
        case MENU_SOUND:
           // setSound(!sound);
            //playSound2();
            break;
        case MENU_RECORD:
            state = STATE_RECORD;
            //playSound2();
            break;
        case MENU_CREDIT:
            state = STATE_CREDIT;
           // playSound2();
            break;
        }
    }
    return ret;
}

//static void setSound(bool enabled)
//{
//}

//static void playSound1(void)
//{
//}

//static void playSound2(void)
//{
//}

//static void readRecord(void)
//{
//}

/*---------------------------------------------------------------------------*/
/*                              Draw Functions                               */
/*---------------------------------------------------------------------------*/

static void drawTitleMenu(void)
{
	const uint8_t *pImg;
    /*  Last score  */
    if (lastScore > 0) {
//        arduboy.printEx(0, 0, F("SCORE"));
//        arduboy.setCursor(0, 6);
//        arduboy.print(lastScore);
		OLED_PrintCharEx(0, 0, "SCORE");
		SetCursor(0, 6);
		OLED_PrintNum(lastScore);
    }

    /*  Man  */
    int16_t y = 32 + manVy * manVy / 32;
    if (y < 64) {
        OLED_DrawBitmap(manX, y, imgMan[manType], 32, 32, WHITE);
    }

    /*  "Hopper" letters  */
    for (int i = 0, x = 22; i < 6; i++) {
      //  *pImg = pgm_read_word(&(letterInfos[i].pImg));
			  pImg = (letterInfos[i].pImg);
        uint8_t w = pgm_read_byte(&(letterInfos[i].width));
        OLED_DrawBitmap(x, letterY[i] / 4 + (i > 0) * 5, pImg, w, 16, WHITE);
        x += w + 1;
    }

    /*  Menu items  */
    char buf[12];
    const char *p = menuText;
    for (int i = 0; i < 4; i++) {
        strcpy_P(buf, p);
        p += OLED_PrintCharEx(68 - (i == menuPos) * 4, i * 6 + 40, buf) + 1;
        if (i == MENU_SOUND) {
            OLED_PrintChar((sound) ? F("ON") : F("OFF"));
        }
    }
    FillRect2(56, menuPos * 6 + 40, 5, 5, WHITE);
}

static void drawTitleRecord(void)
{
    OLED_PrintCharEx(22, 4, F("BEST 10 SCORES"));
    OLED_DrawFastHLine(0, 12, 128, WHITE);
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 5; j++) {
            int r = i * 5 + j;
            OLED_PrintCharEx(i * 60 + 4 - (r == 9) * 6, j * 6 + 14, F("["));
            OLED_PrintNum(r + 1);
            OLED_PrintChar(F("] "));
            OLED_PrintNum(hiScore[r]);
        }
    }
    OLED_DrawFastHLine(0, 44, 128, WHITE);
    OLED_PrintCharEx(16, 48, F("PLAY COUNT "));
    OLED_PrintNum(playCount);
    OLED_PrintCharEx(16, 54, F("PLAY TIME  "));
    OLED_PrintNum(playFrames / 3600); // minutes
    char buf[6];
    sprintf(buf, "'%02d\"", playFrames / 60 % 60); // seconds
    OLED_PrintChar(buf);
}
size_t strnlen(const char *s, size_t count)
{
    const char *sc;
 
    for (sc = s; count-- && *sc != '\0'; ++sc);
    return sc - s;
}

static void drawTitleCredit(void)
{
    char buf[20];
    const char *p = creditText;
    for (int i = 0; i < 8; i++) {
        strcpy_P(buf, p);
        uint8_t len = strnlen(buf, sizeof(buf));
        p += OLED_PrintCharEx(64 - len * 3, i * 6 + 8, buf) + 1;
    }
}
